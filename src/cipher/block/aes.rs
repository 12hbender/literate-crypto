...
impl BlockCipher for Aes128 {
    ...
    fn decrypt(_data: Ciphertext<Self::Block>, _key: Key<Self::Key>) -> Plaintext<Self::Block> {
        todo!()
    }
}
...
fn mix_columns<const BLOCK_BYTES: usize>(state: &mut [u8; BLOCK_BYTES]) {
    ...
    state.chunks_mut(4).zip(copy.chunks(4)).for_each(|(s, c)| {
        s[0] = x_time(c[0]) ^ x_time_3(c[1]) ^ c[2] ^ c[3];
        s[1] = c[0] ^ x_time(c[1]) ^ x_time_3(c[2]) ^ c[3];
        s[2] = c[0] ^ c[1] ^ x_time(c[2]) ^ x_time_3(c[3]);
        s[3] = x_time_3(c[0]) ^ c[1] ^ c[2] ^ x_time(c[3]);
    });
}
